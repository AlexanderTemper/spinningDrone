...
void mspSerialAllocatePorts(void) {
    ...
    while (portConfig && portIndex < MAX_MSP_PORT_COUNT) {
        ...
        if (serialPort) {
            resetMspPort(mspPort, serialPort, false);
            portIndex++;
        }
...
static mspPostProcessFnPtr mspSerialProcessReceivedCommand(mspPort_t *msp, mspProcessCommandFnPtr mspProcessCommandFn) {
    ...
    mspPacket_t reply = { .buf = { .ptr = outBuf, .end = ARRAYEND(outBuf), }, .cmd = -1, .flags = 0, .result = 0, .direction = MSP_DIRECTION_REPLY, };
...
static void mspSerialProcessReceivedReply(mspPort_t *msp, mspProcessReplyFnPtr mspProcessReplyFn) {
    mspPacket_t reply = { .buf = { .ptr = msp->inBuf, .end = msp->inBuf + msp->dataSize, }, .cmd = msp->cmdMSP, .result = 0, };
