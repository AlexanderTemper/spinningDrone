...
uint8_t writeBuffer[5]; //i2c Write buffer
...
uint8_t getSpadInfo(uint8_t * count, bool * type_is_aperture);
...
VL53L0X_Error readMulti(uint8_t reg, uint8_t *dst, uint8_t count) {
...
VL53L0X_Error writeReg(uint8_t reg, uint8_t value) {
    return writeMulti(reg, &value, 1);
}

// Write a 16-bit register
VL53L0X_Error writeReg16Bit(uint8_t reg, uint16_t value) {
    uint8_t buff[2];
    buff[1] = value & 0xFF;
    buff[0] = value >> 8;
    return writeMulti(reg, buff, 2);
}

// Write a 32-bit register
VL53L0X_Error writeReg32Bit(uint8_t reg, uint32_t value) {
    uint8_t buff[4];
    buff[3] = value & 0xFF;
    buff[2] = value >> 8;
    buff[1] = value >> 16;
    buff[0] = value >> 24;
    return writeMulti(reg, buff, 4);
}
// Read an 8-bit register
uint8_t readReg(uint8_t reg) {
    uint8_t buffer;
    readMulti(reg, &buffer, 1);
    return buffer;
}
...
float getSignalRateLimit(void) {
    float limit_Mcps = readReg16Bit(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT);
    return limit_Mcps / (1 << 7);
}
