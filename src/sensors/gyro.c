...
static FAST_RAM_ZERO_INIT bool useDualGyroDebugging = true;
...
bool gyroInit(void)
{
    ...
    if (gyroDetectSensor(&gyroSensor1)) {
...
static FAST_CODE FAST_CODE_NOINLINE void gyroUpdateSensor(gyroSensor_t *gyroSensor, timeUs_t currentTimeUs)
{
    ...
    timeDelta_t delta = cmpTimeUs(lastGyroReadTime,currentTimeUs);
    lastGyroReadTime = currentTimeUs;
    ...
    if (gyroDebugMode == DEBUG_NONE) {
        filterGyro(gyroSensor,delta);
    } else {
        filterGyroDebug(gyroSensor,delta);
    }
}
...
bool gyroGetAccumulationAverage(float *accumulationAverage)
{
    if (accumulatedMeasurementTimeUs > 0) {
        ...
        DEBUG_SET(DEBUG_GYRO_FILTERED, 3, lrintf(accumulationAverage[2]));
